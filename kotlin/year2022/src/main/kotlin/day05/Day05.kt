...
fun mover9001(src: Stack<Crate>, dest: Stack<Crate>, amount: Int) {
  val temp = buildList<Crate> { repeat(amount) { add(src.pop()) } }
  temp.reversed().forEach { dest.push(it) }
}

fun List<Stack<Crate>>.top(): String = map(Stack<Crate>::peek).joinToString("")

fun parseStacks(input: String): List<Stack<Crate>> {
  val lines = input.lines().takeWhile { it.contains('[') }.reversed()
