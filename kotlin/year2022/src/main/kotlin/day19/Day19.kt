...
fun parseInput(input: List<String>): List<Blueprint> = input.map(::parseBlueprint)
...
fun solvePartTwo(blueprints: List<Blueprint>): Int =
  blueprints
    .take(3)
    .map { it.qualityLevel(targetMineral = Geode, timeLimit = 32) }
    .reduce(Int::times)
...
data class Blueprint(val costsForRobots: Map<Mineral, Map<Mineral, Int>>) {
  fun maxCost(mineral: Mineral): Int = costsForRobots.values.maxOf { it[mineral] ?: 0 }
...
data class Stock(
  ...
  fun amountOfMineral(mineral: Mineral): Int = minerals[mineral] ?: 0
  ...
  private fun robotIsNeeded(mineralToCollect: Mineral): Boolean =
    mineralToCollect == targetMineral ||
      (robots[mineralToCollect] ?: 0) < blueprint.maxCost(mineralToCollect)
  ...
  private fun timeNeededToBuildRobot(mineralToCollect: Mineral): Int {
    val max =
      blueprint.costsForRobots[mineralToCollect]?.maxOf { (mineral, cost) ->
  ...
  private fun stockAfterRobotWasBuilt(mineralToCollect: Mineral): Stock {
    ...
    val stock =
      Stock(
        ...
        time = time + buildTime,
        robots =
          robots.mapValues { (mineral, amount) ->
            if (mineral == mineralToCollect) amount + 1 else amount
          },
        minerals =
          minerals.mapValues { (mineral, amount) ->
