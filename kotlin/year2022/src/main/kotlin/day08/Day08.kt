...
fun parseInput(input: String): Forest =
  input.trim().lines().map { line -> line.map(Char::digitToInt) }
...
fun Forest.visibilityScore(): Int {
  val count =
    (1 until width - 1)
      .flatMap { x -> (1 until hight - 1).map { y -> isVisibleAt(x, y) } }
      .count { it }
...
fun Forest.scenicScore(): Int =
  (1 until width - 1).flatMap { x -> (1 until hight - 1).map { y -> scenicScoreAt(x, y) } }.max()
