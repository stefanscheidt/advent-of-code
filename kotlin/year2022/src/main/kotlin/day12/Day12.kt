...
fun List<String>.find(char: Char): Point2D? {
  forEachIndexed { y, row -> if (row.contains(char)) return Point2D(row.indexOf(char), y) }
  return null
}
...
data class Heightmap(val heights: List<List<Int>>, val start: Point2D, val dest: Point2D) {
  fun contains(point: Point2D): Boolean =
    point.y in heights.indices && point.x in heights[point.y].indices

  fun heightAt(point: Point2D): Int = heights[point.y][point.x]
}

fun Point2D.neighbours(): Set<Point2D> =
  setOf(copy(x = x - 1), copy(x = x + 1), copy(y = y - 1), copy(y = y + 1))

data class PointWithCost(val point: Point2D, val cost: Int) : Comparable<PointWithCost> {
...
fun Heightmap.shortestPathCost(
  ...
  while (costs.isNotEmpty()) {
    ...
    if (next.point !in seen) {
      seen.add(next.point)
      val reachable = next.point.neighbours().filter { contains(it) && isReachable(next.point, it) }
