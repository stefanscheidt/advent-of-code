...
fun solvePartOne(packets: List<Packet>): Int =
    packets.chunked(2)
        .mapIndexed { index, chunk -> if (chunk[0] < chunk[1]) index + 1 else 0 }
        .sum()
...
data class ListPacket(val elements: List<Packet>) : Packet() {
    override fun compareTo(other: Packet): Int {
        return when (other) {
            ...
            is ListPacket -> elements.zip(other.elements)
...
object PacketGrammar : Grammar<Packet>() {
    ...
    private val nonEmptyList by (
            skip(lbracket) and separatedTerms(number or parser(this::rootParser), comma) and skip(rbracket)
            ) map { ListPacket(it) }
