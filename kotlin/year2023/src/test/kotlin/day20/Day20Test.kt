...
class Day20Test {
    ...
    fun `broadcaster receives pulse`() {
        ...
        config.receive(Pulse(src = "test", dest = "broadcaster", High)) shouldBe listOf(
            Pulse("broadcaster", "a", High),
            Pulse("broadcaster", "b", High)
        )
    }
    ...
    fun `flip-flop receives pulse`() {
        ...
        config.receive(Pulse(src = "test", dest = "a", Low)) shouldBe listOf(
            Pulse("a", "inv", High),
            Pulse("a", "con", High)
        )
