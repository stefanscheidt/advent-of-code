...
fun solvePartOne(bricks: List<Brick>): Int =
    bricks.settled().count { it.isSafeToDesintegrate() }
...
fun parseBricks(input: List<String>): List<Brick> =
    input.map(::parseBrick).sortedBy { it.zs.first }
...
data class Brick(val xs: IntRange, val ys: IntRange, val zs: IntRange) {
    ...
    infix fun supports(other: Brick): Boolean =
        this overlapps other && zs.last == other.zs.first - 1

    fun moveTo(z: Int): Brick =
        copy(zs = zs.moveTo(z))
    ...
    fun isSafeToDesintegrate(): Boolean =
...
fun List<Brick>.settled(): List<Brick> {
    ...
    fallingBricks.forEach { brick ->
        val settledBrick = if (settledBricks.any { it supports brick }) {
            brick
        } else {
            val maxZBelow = settledBricks.filter { it willSupport brick }.maxOfOrNull { it.zs.last } ?: 0
...
infix fun IntRange.overlapps(other: IntRange): Boolean =
    other.last >= first && other.first <= last

val IntRange.width: Int
    get() = last - first

fun IntRange.moveTo(x: Int) =
    (x..x + width)
