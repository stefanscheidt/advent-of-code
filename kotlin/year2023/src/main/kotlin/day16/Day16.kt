...
fun Beam.radiate(direction: Point2D): Beam = copy(pos = pos + direction, direction = direction)

typealias Grid = List<String>

fun Grid.contains(pos: Point2D): Boolean = pos.x in (0 until first().length) && pos.y in indices
...
fun Grid.energize(start: Beam): Set<Point2D> {
  ...
  while (beams.isNotEmpty()) {
    beams.removeFirst().radiate(this).forEach { beam ->
      if (beam !in seen) {
        seen.add(beam)
        beams.add(beam)
      }
