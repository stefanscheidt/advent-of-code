...
fun solvePartOne(input: String): Int = input.rows().map(String::tiltStart).totalLoadNorth
...
fun String.tiltNorth(): String = rows().map(String::tiltStart).rowJoin()

fun String.tiltEast(): String = lines().map(String::tiltEnd).lineJoin()

fun String.tiltSouth(): String = rows().map(String::tiltEnd).rowJoin()

fun String.tiltWest(): String = lines().map(String::tiltStart).lineJoin()

fun String.cycle(): String = tiltNorth().tiltWest().tiltSouth().tiltEast()
...
fun List<String>.lineJoin(): String = joinToString(separator = System.lineSeparator())

fun List<String>.rowJoin(): String =
  (0 until maxOf(String::length)).joinToString(separator = System.lineSeparator()) { row ->
