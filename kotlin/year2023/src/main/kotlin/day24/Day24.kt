...
fun solvePartOne(hailstones: List<Hailstone>, range: LongRange): Int {
    ...
    for (i in (hailstones.indices.first until hailstones.indices.last)) {
        for (j in (i + 1..hailstones.indices.last)) {
            ...
            val l1 = (v2.y * (p1.x - p2.x) - v2.x * (p1.y - p2.y)).toDouble() / (v1.y * v2.x - v1.x * v2.y).toDouble()
            val l2 = (v1.y * (p2.x - p1.x) - v1.x * (p2.y - p1.y)).toDouble() / (v1.x * v2.y - v1.y * v2.x).toDouble()
...
infix fun Double.inLongRange(range: LongRange): Boolean =
    this >= range.first && this <= range.last
