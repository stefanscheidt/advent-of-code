...
val cardStrengths =
  "23456789TJQKA".toCharArray().mapIndexed { index, char -> char to index + 2 }.toMap()

val Char.strength: Int
  get() = cardStrengths[this] ?: 0

val cardStrengthsWithJoker =
  "J23456789TQKA".toCharArray().mapIndexed { index, char -> char to index + 1 }.toMap()
...
val handComparator: Comparator<Hand> = Comparator { h1, h2 ->
  ...
  if (s1 != s2) {
    ...
    h1
      ...
      ?.let { it.first.strength.compareTo(it.second.strength) } ?: 0
  }
}

val handComparatorWithJoker: Comparator<Hand> = Comparator { h1, h2 ->
  ...
  if (s1 != s2) {
    ...
    h1
      .zip(h2)
      .find { it.first != it.second }
      ?.let { it.first.strengthWithJoker.compareTo(it.second.strengthWithJoker) } ?: 0
  }
}
