...
fun parseGame(input: String): Game {
  ...
  val (id) =
    gameRegex.find(input.substringBefore(":"))?.destructured ?: error("invalid game [$input]")
...
fun parseHands(input: String): List<Hand> = input.split(";").map(::parseHand)

// example input:
// 3 blue, 4 red
fun parseHand(input: String): Hand = buildMap {
...
fun solvePart1(games: List<Game>): Int =
  games.filter { it.isPossible(red = 12, green = 13, blue = 14) }.sumOf { it.id }

fun solvePart2(games: List<Game>): Int = games.sumOf(Game::power)
...
fun Hand.amountOf(color: Color): Int = this[color] ?: 0

fun List<Hand>.minOf(color: Color): Int = this.maxOf { it.amountOf(color) }

data class Game(val id: Int, val hands: List<Hand>) {

  fun isPossible(red: Int, green: Int, blue: Int): Boolean =
    hands.all {
      it.amountOf(Red) <= red && it.amountOf(Green) <= green && it.amountOf(Blue) <= blue
    }
