...
fun solvePartOne(workflows: Map<String, Workflow>, ratings: List<Rating>): Int =
    ratings.map { it to workflows.assess(it) }
...
fun parseRating(input: String): Rating {
    val (x, m, a, s) = ratingRegex.matchEntire(input)?.destructured ?: error("invalid rating [$input]")
...
fun parseWorkflow(input: String): Workflow {
    val (name, rulesInput) = workflowRegex.matchEntire(input)?.destructured ?: error("invalid workflow [$input]")
